...
const HUMAN_STEPS = [
  ...
  { path:"iss-n-r",from:"issuer",  to:"network",   label:"approved",       status:"pass", info:"Approved · authorization metadata returned" },
  { path:"n-acq-r",from:"network", to:"acquirer",  label:"response",       status:"pass", info:"Authorization response confirmed" },
...
const FIXED_STEPS = [
  ...
  { path:"agent-tap",  from:"agent",to:"network",  label:"VIC: get credentials", status:"pass",
    ...
    info:"✓ Agent obtains a scoped, tokenized payment credential through a controlled payment service\n✓ Public sources support tokenization and consumer controls\n✓ Exact Visa API names, endpoint paths, and payloads are not public specs\n✓ Mastercard equivalent: tokenized agent credential through its Agent Pay architecture" },
  { path:"agent-m",    from:"agent",to:"merchant",  label:"HTTP + TAP signature", status:"pass",
    layer:"Merchant + Gateway",
    info:"✓ Agent signs HTTP request with Ed25519-style HTTP Message Signatures (RFC 9421)\n✓ Signature-Input + Signature headers bind method, target, digest, timestamp, nonce, and action tag\n✓ Edge or merchant backend verifies against a registered public key\n✓ Exact headers, cache timing, and verdict format are implementation-specific" },
  { path:"m-acq",      from:"merchant",to:"acquirer",label:"PaymentIntent",    status:"pass",
    layer:"Acquirer + Processor",
    info:"✓ Payment request uses a tokenized credential plus agent/payment context\n→ Exact ISO 8583 data elements and private fields are network implementation details" },
  { path:"acq-n",      from:"acquirer",to:"network", label:"authorization context", status:"pass",
    layer:"Card Network",
    info:"✓ Network can compare token, merchant, amount, and delegated policy context\n→ Public sources do not disclose exact private validation fields or response-code behavior" },
  { path:"n-iss",      from:"network", to:"issuer",  label:"agent policy",     status:"pass",
    layer:"Issuer Bank",
    info:"✓ Issuer can evaluate spending policy, category, velocity, token status, and authentication history\n→ Approved agent checkout does not have to look like unknown bot traffic" },
  { path:"iss-n-r",    from:"issuer",  to:"network", label:"approved",          status:"pass",
    info:"Approved · issuer authorization metadata returned through existing rails" },
  { path:"n-acq-r",    from:"network", to:"acquirer",label:"response",         status:"pass",
    info:"Authorization response returned to acquirer" },
