...
const LAYERS = [
  {
    ...
    sub: "ISO 8583 net settlement · L3 line items · data enrichment",
    color: "var(--diagram-2)",
    ...
    protocols: [
      ...
      { name: "Data enrichment", coverage: "Structured transaction evidence for reconciliation and review" },
    ],
    vendors: ["Corporate AP systems", "Issuers", "Agent platforms"],
    field: null,
    fact: "L3 data historically < 20% populated by humans. AI agents generate it for free as a byproduct of invoice extraction. data enrichment makes this financially rewarding for merchants.",
  },
  {
    id: 4,
    name: "Payment Rail",
    sub: "ISO 8583 auth · authorization metadata · lifecycle links",
    color: "var(--success)",
    ...
    tag: "L4",
    description: "The authorization message still rides existing card-network rails. Agent identity, token status, and delegation evidence can become authorization metadata, but public sources do not publish a complete field-level map for agentic card transactions.",
    protocols: [
      { name: "TAP trust evidence", coverage: "Signed-request verification can inform authorization context; exact Visa message fields are not public." },
      { name: "AP2 PaymentMandate", coverage: "Mandate evidence can accompany payment execution through method-specific extensions." },
      { name: "Lifecycle identifiers", coverage: "Related authorization, clearing, refund, and dispute messages can link back to the original authorization event" },
    ],
    vendors: ["card network", "Mastercard MDES", "Adyen", "Worldpay"],
    field: "token metadata · authorization context · lifecycle link",
    fact: "The safe public claim is backward compatibility with existing rails. Exact field numbers, private elements, and response codes should stay out of public diagrams unless sourced.",
  },
  {
    id: 3,
    ...
    vendors: ["Corporate AP systems", "Wallets", "Agent platforms"],
    field: null,
  ...
  },
  {
    ...
    sub: "Intent mandates · cart mandates · signed credentials · OAuth grant tokens",
    color: "var(--diagram-1)",
    ...
    protocols: [
      ...
      { name: "Mastercard Verifiable Intent", coverage: "Delegation credential model for constrained user-to-agent and agent-to-network authority." },
    ...
    vendors: ["Google (AP2)", "Mastercard (VI)", "Grantex (DAAP)", "agent platform"],
    field: null,
  ...
  },
  {
    ...
    tag: "L1",
    description: "Before any payment, the system must answer: who is this agent, and who authorized it? Two approaches dominate. Cryptographic key-based identity uses registered keys and HTTP Message Signatures. Registry-based identity binds recognized agent platforms to token and acceptance infrastructure.",
    protocols: [
      { name: "Visa TAP (Oct 2025)", coverage: "RFC 9421 HTTP signatures with public-key lookup, freshness checks, nonce handling, and action tags." },
    ...
    vendors: ["Visa (agent registry)", "Mastercard (MDES)", "agent platform (agent payment flow)", "platform"],
