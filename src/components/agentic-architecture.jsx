  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row" }}>
          ...
          <div style={{ padding: "12px 16px 4px" }}>
            ...
            {ACTOR_ORDER.map((id, index) => {
              ...
              return (
                <div key={id}>
                  {showBand && (
                    <div style={{ fontSize: 9, fontFamily: "ui-monospace,monospace", color: "#b8a898", letterSpacing: "0.08em", textTransform: "uppercase", marginTop: index === 0 ? 0 : 10, marginBottom: 4 }}>
                      {actor.band}
                    </div>
                  )}
                  <div
                    ...
                    style={{
                      ...
                      border: `1px solid ${isSelected ? accent : "#ddd8ce"}`,
                  ...
                  >
                    ...
                    <div style={{ fontSize: 12, color: isSelected ? accent : "#ddd8ce" }}>{isSelected ? "▲" : "▼"}</div>
                  </div>
                  {isSelected && actor.detail && (
                    <div style={{ margin: "0 0 8px 14px", padding: "12px 14px", background: "#f5f2ec", borderRadius: "0 0 8px 8px", borderLeft: `2px solid ${accent}` }}>
                      <p style={{ fontSize: 12, color: "#5a4f44", lineHeight: 1.7, margin: "0 0 10px" }}>{actor.detail.body}</p>
                      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                        {actor.detail.facts.map((fact, factIndex) => (
                          <div key={factIndex} style={{ display: "flex", gap: 7, alignItems: "flex-start" }}>
                            ...
                            <div style={{ fontSize: 10.5, fontFamily: "ui-monospace,monospace", color: "#7a6e64", lineHeight: 1.5 }}>{fact}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <g key={`${connection.from}-${connection.to}`} style={{ opacity: isFaded ? 0.1 : 1, transition: "opacity 0.25s" }}>
                    ...
                    <g style={{ opacity: isFaded ? 0 : 1, transition: "opacity 0.2s" }}>
                      <rect
                        ...
                        stroke={isRelated ? edgeStyle.stroke : "#ddd8ce"}
                        strokeWidth={isRelated ? 1 : 0.8}
                        strokeOpacity={isRelated ? 0.7 : 0.6}
                      />
                      <text x={mx} y={my + 0.5} textAnchor="middle" dominantBaseline="central" style={{ fontSize: 8, fontFamily: "ui-monospace,monospace", fill: isRelated ? edgeStyle.label : "#a89880", letterSpacing: "0.02em" }}>
                        {connection.label}
                      </text>
                    </g>
                  </g>
                ...
                return (
                  ...
                  >
                    ...
                    <rect
                      ...
                      height={NODE_H}
                      rx={9}
                      fill={isSelected ? "#ffffff" : isHovered ? "#fcfaf7" : "#fdfcf9"}
                      stroke={isSelected ? accent : isHovered ? "#ccc5b8" : "#ddd8ce"}
                    ...
                    <text x={x + 37} y={y + 27} dominantBaseline="central" style={{ fontSize: 12, fontWeight: 600, fontFamily: "system-ui,sans-serif", fill: isSelected ? "#1a1410" : isHovered ? "#2a2218" : "#3a3028", letterSpacing: "-0.01em", transition: "fill 0.15s" }}>
                      {actor.label}
                    </text>
                    <text x={x + 37} y={y + 46} dominantBaseline="central" style={{ fontSize: 8.5, fontFamily: "ui-monospace,monospace", fill: isSelected ? "#8a7868" : "#b8a898", letterSpacing: "0.01em", transition: "fill 0.15s" }}>
                      {actor.sub}
                    </text>
                    {!selected && (
        ...
        {!isMobile && (
          <div style={{ width: 236, borderLeft: "1px solid #e8e3da", background: "#f5f2ec", padding: "20px 18px", flexShrink: 0 }}>
            {detail && selActor ? (
              <div style={{ animation: "fadeUp 0.2s ease" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                  {detail.facts.map((fact, index) => (
                    <div key={index} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                      ...
                      <div style={{ fontSize: 10.5, fontFamily: "ui-monospace,monospace", color: "#7a6e64", lineHeight: 1.55 }}>{fact}</div>
                    </div>
                  ))}
                </div>
                ...
                <button
                  ...
                  onMouseEnter={(event) => {
                    event.target.style.borderColor = "#b8a898"
                    event.target.style.color = "#6a5e54"
                  }}
                  onMouseLeave={(event) => {
                    event.target.style.borderColor = "#d8d3c8"
                    event.target.style.color = "#a89880"
                  }}
                >
                  ← deselect
                </button>
                  ...
                  {ACTOR_ORDER.map((id) => {
                    ...
                    return (
                      <div
                        ...
                        onMouseEnter={(event) => { event.currentTarget.style.background = "#ede8e0" }}
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 11, fontWeight: 500, color: "#4a4038" }}>{actor.label}</div>
                          <div style={{ fontSize: 9, fontFamily: "ui-monospace,monospace", color: "#c8b8a2", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                            {actor.sub}
                          </div>
                        </div>
                      </div>
                ...
                <div style={{ marginTop: 18, paddingTop: 14, borderTop: "1px solid #e0dbd0" }}>
                  {[
                    ...
                    <div key={`${value}-${label}`} style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                      <span style={{ fontSize: 10, fontFamily: "ui-monospace,monospace", color: "#8a7868" }}>{value}</span>
                      <span style={{ fontSize: 9, fontFamily: "ui-monospace,monospace", color: "#c8b8a2" }}>{label}</span>
                    </div>
                  ))}
                </div>
              </div>
